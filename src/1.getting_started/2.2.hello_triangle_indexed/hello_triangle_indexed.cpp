...
// Learn OpenGL 2.2 Hello triangle indexed
...
int main(){
    ...
    // Element buffer object를 이용해 정점들의 데이터와 인덱스를 정리할 수 있다. 
    unsigned int EBO;
    // 버퍼 id를 생성한다. 
    glGenBuffers(1, &EBO);
    ...
    // 즉 EBO를 쓰려면 VBO도 필요한 것이다. VBO --> Vertex 4개 복사
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);

    // GL_ELEMENT_ARRAY_BUFFER로 지정한다.
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    // 그러면 우리가 호출하는 모든 버퍼는 EBO를 타겟으로 하게 된다.

    // 버퍼의 유형에 대해, 사용자가 정의한 **indeces의** 데이터를 복사한다. 이 때 static하다는 것을 알려준다. 
    // EBO --> 어떤 index가 삼각형을 이루는지에 대한 데이터 2개 복사
