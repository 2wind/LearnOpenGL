...
// Learn OpenGL 3.1 Shaders uniform
...
// in 으로 정의된 변수로 값을 받을 수 있고, out으로 정의된 변수를 내보낸다.
// 이건 셰이더 공통. 응용프로그램에서 셰이더에 값을 보내려면 Uniforms를 이용한다.
// 이 uniform은 전역변수로, 우리가 원하는 모든 shader에서 선언이 가능하다.
...
int main(){
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        float greenValue = (sin(timeValue) / 2.0f) + 0.5f; // 0과 1 사이 진동함.
        // ourcolor에 해상하는 값을 가져와서 vertexColorLocation에 할당.
        int vertexColorLocation = glGetUniformLocation(shaderProgram, "ourColor"); 

        glUseProgram(shaderProgram);            // 2. 이제 위에서 만든 shaderProgram을 써서,
        // 셰이더 내부의 ourColor를 가리키는 vertexColorLocation을 이용해 vec4 값을 할당한 다음,
