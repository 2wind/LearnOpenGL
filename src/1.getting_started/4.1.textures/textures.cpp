...
int main(){
    ...
    Shader ourShader("../../src/1.getting_started/4.1.textures/shader.vs", "../../src/1.getting_started/4.1.textures/shader.fs");
    // ==================== 초기화 부분 끝 ===========================
    ...
    // 텍스처 좌표가 0과 1 사이가 아닐 때 어떻게 할지 처리한다.
    // 2D 텍스처에 대해, S축(x축)상에서 거울 반복으로 한다. T축도 마찬가지.
    // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_MIRRORED_REPEAT);
    // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT);

    // 테두리로 clamp할 시 테두리 색도 설정한다.
    // float borderColor[] = { 1.0f, 1.0f, 0.0f, 1.0f };
    // glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, borderColor);  

    // 텍스처 필터링 방식을 설정한다.
    // 축소될 때에는 nearest neighbor, 확대시에는 linear interpolation 이용.
    // 여기에 mipmap을 지정할 수도 있다.
    ...
    if (data)
    {
        ...
        // GL_TEXTURE_2D: 텍스쳐 타겟으로 2D 텍스쳐를 지정한다.
        // 0 : 밉맵 레벨을 수동으로 지정한다.
        // GL_RGB: 우리는 텍스처가 RGB 포맷이었으면 한다.
        // width, height: 텍스처의 높이와 너비(stbi_load()에서 건네받음).
        // 0 : 무조건 0으로 지정한다.
        // GL_RGB: 우리의 이미지에는 RGB 정보만 있다.
        // GL_UNSIGNED_BYTE: 데이터타입이 char * 타입이다.
        // data : 실제 데이터로 이어지는 값을 지정한다.
